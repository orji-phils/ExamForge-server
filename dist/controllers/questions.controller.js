...
exports.uploadPastQuestion = exports.deletePastQuestion = exports.getRandomQuestions = exports.getPastQuestionById = exports.getPastQuestionByYear = exports.getYears = exports.getTables = exports.getDatabases = void 0;
const structureQuestion_1 = require("../service/utility/structureQuestion");
...
const getDatabases = async (req, res, next) => {
    try {
        ...
        const [fetchDatabases] = await connect.query("SHOW DATABASES");
        ...
        res.status(200).json(question_schema_1.multiExamTypeSchema.parse(relevantDBs));
    }
    catch (error) {
        next({
            status: 500,
...
const getTables = async (req, res, next) => {
    try {
        ...
        const [fetchTables] = await connect.query("SHOW TABLES");
        ...
        // send out server response
        res.status(200).json(question_schema_1.multiSubjectSchema.parse(allTables));
    }
    catch (error) {
        next({
            status: 500,
...
const getYears = async (req, res, next) => {
    try {
        ...
        const connect = await req.connect;
        const [years] = await connect.query("SELECT DISTINCT year FROM ?? ORDER BY year ASC", [subject]);
        ...
        // }
        // send out server response
        res.status(200).json(question_schema_1.multiYearSchema.parse(allYears));
    }
    catch (error) {
        next({
            status: 500,
...
const getPastQuestionById = async (req, res, next) => {
    if (!req.user) {
        return next({
            status: 401,
            severity: "high",
    ...
    try {
        ...
        // send out server response
        res.status(200).json(question_schema_1.multiQuestionSchema.parse(questions));
    }
    catch (error) {
        next({
            status: 500,
...
exports.getPastQuestionById = getPastQuestionById;
...
const getRandomQuestions = async (req, res, next) => {
    try {
        ...
        const queries = subjects.map(subject => connect.query(`SELECT * FROM ??
            WHERE id >= FLOOR(RAND() * 
            (SELECT MAX(id) FROM ??))
        ...
        // send out server response
        res.status(200).json(question_schema_1.multiQuestionSchema.parse(randomQuestions));
    }
    catch (error) {
        next({
            status: 500,
...
const getPastQuestionByYear = async (req, res, next) => {
    try {
        const { year, subject } = req.params;
        const connect = await req.connect;
        // fetch the expected past question
        const [pastQuestion] = await connect.query(`SELECT * FROM ?? WHERE year = ? ORDER BY questionNumber ASC`, [subject, year]);
        // send out server response
        res.status(200).json(question_schema_1.multiQuestionSchema.parse(pastQuestion));
    }
    catch (error) {
        next({
            status: 500,
...
exports.getPastQuestionByYear = getPastQuestionByYear;
// delete all of the past question with the same year and subject.
const deletePastQuestion = async (req, res, next) => {
    ...
    try {
        ...
        // get deleters data
        const [deleter] = await userConnect.query(`SELECT userName FROM Users 
            WHERE id = ?`, [req.user.id]);
        const user = deleter[0];
        // check if question exists
        const [existingQuestions] = await connect.query("SELECT * FROM ?? WHERE year = ?", [subject, year]);
        ...
        await connect.query("DELETE FROM ?? WHERE year = ?", [subject, year]);
        // log the action
        (0, createLogs_1.auditLog)({
            performedBy: req.user.id,
            actionType: "Questions Deleted",
...
exports.deletePastQuestion = deletePastQuestion;
// create new or update existing past question
const uploadPastQuestion = async (req, res, next) => {
    if (req.user.role === "user") {
        return next({
            status: 403,
            severity: "high",
    ...
    const connect = await pool.getConnection();
    try {
        const { subject, year } = question_schema_1.questionFieldSchema.parse(req.params);
        const sentQuestions = question_schema_1.multiQuestionSchema.parse(await (0, structureQuestion_1.spreadData)(req, res, next));
        const userConnect = await dbConnect_1.userDB;
        // get uploader detail
        const [uploader] = await userConnect.query(`SELECT userName, id FROM Users 
            WHERE id = ?`, [req.user.id]);
        ...
        const [existingQuestions] = await connect.query(`SELECT id FROM ?? 
        ...
        if (existingQuestions.length) {
            ...
            const updatedPastQuestions = sentQuestions.map(async (question, index) => {
                return connect.query(`UPDATE ?? SET 
                    questionNumber = ?, question = ?, options = ?, correctAnswer = ?, year = ?, subject = ? 
                    WHERE id = ?`, [subject, question.questionNumber, question.question, JSON.stringify(question.options), question.correctAnswer, year, subject, existingQuestions[index]?.id]);
            });
            ...
            const uploadedQuestions = sentQuestions.map(question => {
                return connect.query(`INSERT INTO ?? 
                    (questionNumber, question, options, correctAnswer, year, subject) 
                    VALUES(?, ?, ?, ?, ?, ?)`, [subject, question.questionNumber, question.question, JSON.stringify(question.options), question.correctAnswer, year, subject]);
            });
...
exports.uploadPastQuestion = uploadPastQuestion;
