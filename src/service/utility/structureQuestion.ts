...
import { auditLog } from "./createLogs";
import { userDB } from "../../libs/dbConnect";

const spreadData = async (req: Request, res: Response, next: NextFunction): Promise<PastQuestion[]> => {
    // Initialize result as an array of objects
    const result: PastQuestion[] = [];
    const fullText: string | undefined = req.body?.content || await readFileContent(req, res);
    ...
    if (!fullText) {
        ...
        await auditLog({
            ...
            actionType: "Empty Content",
            description: `${user?.userName} attempted uploading empty questions.`
        });

        res.status(404).json({ message: "Question content not found." });
        return [];
    }
...
const readFileContent = async (req: Request, res: Response): Promise<string> => {
    const filePath: string | undefined = req.file?.path;
    const content = req.body.question;

    if (!filePath && !content) {
        res.status(400).json({ message: "Please upload the intended file, or paste or type the question content." });
        return "";
    }
