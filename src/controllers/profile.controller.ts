...
const getProfile = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        let [userProfile] = await connect.query<Profile[]>(
        ...
        if (role !== "user") {
            const [adminData] = await connect.query<Profile[]>(
                "SELECT * FROM AdminProfile WHERE profileId = ?",
                [profile?.id]
            );
...
const deleteProfile = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const [retrievedProfile] = await connect.query<Profile[]>(
            "SELECT Users.userName, UserProfile.id FROM Users JOIN UserProfile ON users.id = UserProfile.userId WHERE Users.id = ?", [id]
        );
        ...
        const [deleted] = await connect.query<ResultSetHeader>(
...
const createProfile = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const { id, role } = req.user;
        const { firstName, lastName, dateOfBirth, phoneNumber, bio, accountNumber, bankName } = profileSchema.parse(req.body);
        ...
        const [userProfile] = await connect.query<Profile[]>(
            `SELECT u.userName, up.*, ap.* FROM Users u
        ...
        if (profile) {
            ...
            await connect.query(
                ...
                WHERE UserId = ?`, 
                [firstName, lastName, dateOfBirth, picturePath, phoneNumber, bio, id]
            );
            ...
            if (!profile.accountNumber) {
                await connect.query(
                    ...
                    VALUES (?, ?, ?)`,
                    [profile?.id, accountNumber, bankName]
                );
            }
        ...
        await connect.query(
            ...
            WHERE profileId = ?`,
            [accountNumber, bankName, profile?.id]
        );
        ...
        auditLog({
            ...
            const [insertedProfile] = await connect.query<ResultSetHeader>(
                `INSERT INTO UserProfile 
                (userId, firstname, lastName, dateOfBirth, profilePicture, phoneNumber, bio) 
                VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [id, firstName, lastName, dateOfBirth, picturePath, phoneNumber, bio]
            );
            ...
            await connect.query(
                ...
                VALUES (?, ?, ?)`,
                [accountNumber, bankName, insertedProfile.insertId]
            );
