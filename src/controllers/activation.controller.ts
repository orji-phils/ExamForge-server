...
import { activateSchema } from "../schemas/activation.schema";
...
const activateAccount = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const [fetchActivationData] = await connect.query<User[]>(
            `SELECT userName, activationToken, id, role from Users WHERE id = ? AND activationToken = ? AND activationExpiresAt > NOW()`, [userId, hashToken]
        );
...
const deactivateAccount = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const { userId, userName } = activateSchema.parse(req.params);
...
const resendToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        // get the user's data
        const [row] = await connect.query<User[]>(
        ...
        mailer(data?.email!, "Activate Your ExamForge Account", activationMail(data?.userName!, activationLink, year));
        await connect.commit();
